...
export function TabPrestations({ ev, onGoToStands }: { ev: Evenement; onGoToStands: () => void }) {
  ...
  useEffect(() => {
    ...
    sb.from('evenement_prestataires')
      ...
      .then(({ data }) => {
        if (!data) return
        const p = data.map(r => r.prestataires as unknown as Prestataire).filter(Boolean).sort((a, b) => a.raison_sociale.localeCompare(b.raison_sociale, 'fr'))
        if (p.length) {
  ...
  return (
    <>
      ...
      {modal !== null && <PrestationFormOrg prest={modal === 'new' ? null : modal} evenementId={ev.id} onSaved={() => { setModal(null); load() }} onGoToStands={() => { setModal(null); onGoToStands() }} canDelete />}
