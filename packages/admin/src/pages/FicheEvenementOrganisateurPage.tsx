                ...
                {prestations.map(p => (
                  <tr key={p.id} style={{ cursor: 'pointer', borderTop: '1px solid var(--border)', ...conformiteBg(p.statut_conformite) }} onClick={() => setEditingPrestation(p)}>
                    ...
                    <td style={{ padding: '8px 8px 8px 0' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, background: p.statut_conformite === 'conforme' ? '#22c55e' : (p.statut_conformite === 'non_conforme' || p.statut_conformite === 'absent') ? '#f97316' : p.statut_conformite === 'a_verifier' ? '#d97706' : '#9ca3af' }} />
                        {p.libelle}
                      </span>
                    </td>
          ...
          <div className="card-body">
            <DataTable
              ...
              columns={[
                ...
                { key: 'libelle', label: 'Libellé', sortable: true, filterable: true, render: p => (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, background: (p as Prestation).statut_conformite === 'conforme' ? '#22c55e' : ((p as Prestation).statut_conformite === 'non_conforme' || (p as Prestation).statut_conformite === 'absent') ? '#f97316' : (p as Prestation).statut_conformite === 'a_verifier' ? '#d97706' : '#9ca3af' }} title={(p as Prestation).statut_conformite ?? 'Non contrôlée'} />
                    <span style={{ fontWeight: 600 }}>{p.libelle}</span>
                  </span>
                )},
