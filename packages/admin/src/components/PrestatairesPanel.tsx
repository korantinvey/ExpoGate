  ...
  return (
    <Modal title={`Intervenants — ${prestataire.raison_sociale}`} confirmLabel={available.length ? 'Ajouter' : 'Fermer'} onClose={onClose} onConfirm={add}>
      ...
      <p className="text-muted" style={{ marginBottom: 16 }}>
        Ces personnes peuvent ensuite être assignées comme contrôleur prestataire sur un événement.
      </p>
      {contacts.length > 0 ? (
...
export function PrestatairesPanel() {
          ...
          <DataTable
            ...
            emptyState={
              <div className="empty-state">
                ...
                <div>Aucun prestataire</div>
                <div className="mt-4"><button className="btn btn-primary" onClick={() => setModal('new')}>Ajouter un prestataire</button></div>
              </div>
            }
            columns={[
              { key: 'raison_sociale', label: 'Raison sociale', sortable: true, filterable: true, render: p => <span style={{ color: 'var(--accent-dark)', fontWeight: 600 }}>{p.raison_sociale}</span> },
